...
class Home extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({taskList: savedTaskList})
  }
  ...
  onChangeTag = event => {
    ...
    this.setState({
      ...
      activeTag: 'INITIAL', // Reset activeTag when a new tag is selected
    })
  }
  ...
  submitForm = event => {
    ...
    if (inputText.length !== 0) {
      this.setState(prevState => ({
        ...
        inputText: '',
        inputTag: '',
        activeTag: inputTag, // Set activeTag to the newly selected tag
      }))
    }
  }
      ...
      <>
        {filterTaskList.map(each => (
          <TaskListLi key={each.id}>
            ...
            <DeleteButton
              type="button"
              onClick={() => this.deleteTask(each.id)}
            >
              Delete
            </DeleteButton>
          </TaskListLi>
